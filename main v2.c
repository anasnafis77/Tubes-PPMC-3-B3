	...
	switch (menu) 
			{
				...
				case 7 :
						...
						pilihanMenu ();
					break;
				case 8 : saveProject(pcb,fileName,n,m);
						 halamanAwal();
						 pilihanHalaman();
					break;
			}			
...
//	while (!(atoi(&koorX)>0 || atoi(&koorX)<n || atoi(&koorY)>0 ||atoi(&koorY)<m)||koorX=='n'||koorX=='N'||koorX=='q'||koorX=='Q')
//	{
//		printf ("Koordinat %d : ",jumlahKoor);
//		scanf (" %c%*c%c", &koorX,&koorY);
//		if ()
...
//	{
//		while(koorX!='n'||koorX!='N'||koorX!='q'||koorX!='Q')
//		{
//			jumlahKoor++;
//			printf ("Koordinat %d : ",jumlahKoor);
//			scanf ("%c%*c%c", &koorX,&koorY);
